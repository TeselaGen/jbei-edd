...
export class DataGrid {
  ...
  constructor(dataGridSpec: DataGridSpecBase) {
    ...
    this._classes =
      "dataTable sortable dragboxes hastablecontrols table-striped table-bordered";
  ...
  private _createOptionsMenu(): DataGrid {
    ...
    var hasColumnsInVisibilityList: boolean = this._spec.tableColumnGroupSpec.some(
      group => {
    ...
    var menuLabel = $((this._optionsLabel = document.createElement("div")))
      ...
      .text("View options \u25BE")
      .click(() => {
        if (menuLabel.hasClass("pulldownMenuLabelOff")) this._showOptMenu();
      })
      .appendTo(mainSpan);
          ...
          menuColList
            ...
            .each((i, box) => {
              var $box = $(box),
                col = $box.data("column");
              if (
                (data.indexOf(col.name) === -1 && !!col.hiddenByDefault) ||
  ...
  getSelectedCheckboxElements(): HTMLInputElement[] {
    ...
    var filteredSequence = sequence.filter(v => {
      return !!this._recordElements[v];
    });
  ...
  arrangeTableDataRows(): DataGrid {
    ...
    if (!this._groupingEnabled || rowGroupSpec.length < 1) {
      ...
      _.each(rowGroupSpec, grouping => {
        ...
        var replicateIds = this._findReplicateLines(
  ...
  _refreshAllWidgetsWithThrottling() {
    ...
    this._refreshAllWidgets();
    this._widgetRefreshCooldownTimer = setTimeout(
  ...
  private _buildTableHeaders(): HTMLElement[] {
    ...
    this._spec.tableHeaderSpec.forEach((header, i, src) => {
      ...
      var cell: JQuery = $(header.element)
        .css(css)
        .attr({
          id: header.id,
  ...
  private _collapseRowGroup(groupIndex, replicateIds): void {
    ...
    this.scheduleTimer("arrangeTableDataRows", () =>
  ...
  private _clickedColVisibilityControl(
  ...
  private _updateColumnSettings(): DataGrid {
    var propKey = this._columnSettingsKey(),
      setCol = [],
      unsetCol = [],
      delCol = [];
    this._spec.tableColumnGroupSpec.forEach(
    ...
    this._fetchSettings(
      ...
      (data: any) => {
        ...
        data = data.filter((name: string): boolean => {
          return !(
            setCol.indexOf(name.substring(1)) !== -1 && name.indexOf("-") === 0
          );
        });
...
export class DataGridDataCell {
  ...
  createElement() {
    ...
    if (this.checkboxWithID) {
      ...
      var urlSplit = "url: ";
      if (this.contentString.startsWith(urlSplit)) {
        this.contentString = this.contentString.split(urlSplit)[1];
        this.contentContainerElement = $('<a href="https://app.teselagen.com/#strains/' + this.contentString +'" >').appendTo(c)[0];
      } else {
...
export class DGSelectAllWidget extends DataGridHeaderWidget {
  ...
  updateButtonLabel(): void {
    if (this.anySelected) {
      ...
      $(
        "#editButton, #cloneButton, #groupButton, #addAssayButton, #disableButton, #enableButton"
      ).prop("disabled", false);
    } else {
  ...
  testIfAnySelected(): boolean {
    this.anySelected =
      $(this.dataGridSpec.tableElement).find(
...
export class DGSearchWidget extends DataGridHeaderWidget {
  ...
  createElements(uniqueID: string): void {
    var sBoxID: string =
      this.dataGridSpec.tableSpec.id + "SearchBox" + uniqueID;
  ...
  applyFilterToIDs(rowIDs: string[]): string[] {
    ...
    var queryStrs = v.split(" ").filter(one => {
      return one.length > 0;
    });
...
export class DGPagingWidget extends DataGridHeaderWidget {
  ...
  private requestDone: (success: boolean) => void = (
    success: boolean
  ): void => {
    if (success) {
